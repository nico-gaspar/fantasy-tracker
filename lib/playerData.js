...
function computeValueScores(players) {
  // Use players with meaningful game time for normalisation baselines
  const qualified = players.filter(p => (p.gamesPlayed ?? 0) >= 5);
  const maxVol = Math.max(...qualified.map(p => p.totalSeasonPoints ?? 0), 1);

  // Per-position maximums used for normalisation
  const maxPPGByPos       = {};  // for efficiency
  const maxPPMByPos       = {};  // ppg-per-million, for value
  for (const pos of ["GK", "DEF", "MID", "FWD"]) {
    const group = qualified.filter(p => p.position === pos);
    maxPPGByPos[pos] = Math.max(...group.map(p => p.pointsPerGame ?? 0), 1);
    const ppmGroup   = group.filter(p => p.price > 0 && (p.pointsPerGame ?? 0) > 0);
    maxPPMByPos[pos] = ppmGroup.length > 0
      ? Math.max(...ppmGroup.map(p => (p.pointsPerGame ?? 0) / p.price))
      : 1;
  }
  ...
  return players.map(({ _rawPPG, ...p }) => {
    ...
    if (gamesPlayed < 8) {
      // Too few games to assess value reliably
      valueScore = 15;
    } else if (ppg < 2.5) {
      // Low output: heavily penalised, price efficiency can't save them
      const ppgPerMillion = ppg / price;
      valueScore = Math.round((ppgPerMillion / (maxPPMByPos[pos] ?? 1)) * 100 * 0.4);
    } else {
      const isBudget    = price <= 20;
      const isMidRange  = price <= 25;

      const ppgPerMillion    = ppg / price;
      const rawValue         = (ppgPerMillion / (maxPPMByPos[pos] ?? 1)) * 100;
      const budgetMultiplier = isBudget ? 1.20 : isMidRange ? 1.10 : 1.0;
      const gamesBonus       = Math.min(gamesPlayed / 20, 1) * 0.15;

      valueScore = Math.min(Math.round((rawValue * budgetMultiplier) + (gamesBonus * 100)), 100);
    }
