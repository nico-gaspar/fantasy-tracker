...
function weightedFormScore(form) {
  const vals = (form ?? []).filter(v => v != null);
  if (!vals.length) return 0;
  const weights = vals.map((_, i) => Math.pow(2, i));
  const totalW  = weights.reduce((a, b) => a + b, 0);
  return vals.reduce((sum, pts, i) => sum + pts * weights[i], 0) / totalW;
}
...
export function computeValueScores(players) {
  ...
  for (const pos of ["GK", "DEF", "MID", "FWD"]) {
    ...
    const ppg   = (p) => p.pointsPerGame ?? p.pointsPer90 ?? 0;

    maxPPGByPos[pos]  = Math.max(...group.map(p => ppg(p)), 1);
    maxFormByPos[pos] = Math.max(...group.map(p => weightedFormScore(p.form)), 1);

    const adjGroup  = group.filter(p => p.price > 0 && ppg(p) > 0);
    const adjValues = adjGroup.map(
      p => (ppg(p) * Math.max((p.reliability ?? 50) / 100, 0.1)) / p.price
    );
    maxAdjPPMByPos[pos] = Math.max(...adjValues, 0.001);
  }

  // Normalise growthPct within each position group (absorbs recomputeGemScores)
  ...
  return players.map(({ _rawPPG, ...p }) => {
    ...
    const ppg         = p.pointsPerGame ?? p.pointsPer90 ?? 0;
    ...
    if (gamesPlayed < 5) {
      ...
      const ppgScore    = (ppg / (maxPPGByPos[pos] ?? 1)) * 100;
    ...
    let streakScore = 0;
    if (gamesPlayed >= 3 && ppg > 0 && formVals.length > 0) {
      const last5Avg = formVals.reduce((a, b) => a + b, 0) / formVals.length;
      streakScore    = Math.min(100, Math.round((last5Avg / ppg) * 50));
    }
    ...
    if (gamesPlayed < 5) {
      ...
      const price             = Math.max(p.price ?? 1, 0.1);
      const reliabilityFactor = Math.max((p.reliability ?? 50) / 100, 0.1);
      const adjustedPPM       = (ppg * reliabilityFactor) / price;
      const ppmScore          = (adjustedPPM / (maxAdjPPMByPos[pos] ?? 0.001)) * 100;
      const formScore         = (weightedFormScore(p.form) / (maxFormByPos[pos] ?? 1)) * 100;
      const growth            = p.growthPct ?? 0;
      const risingBonus       = (growth > 0 && price < 30)
        ? Math.min((growth / 10) * ((30 - price) / 30) * 25, 20)
        : 0;
      const ppgScore          = (ppg / (maxPPGByPos[pos] ?? 1)) * 100;
      const rawValue          = formScore * 0.40 + ppmScore * 0.35 + ppgScore * 0.25;
      valueScore              = Math.min(Math.round(rawValue + risingBonus), 100);
    }

    // ── Growth score (normalised) ────────────────────────────────────────────
    const rawGrowth   = p.growthPct ?? 0;
    const growthScore = rawGrowth <= 0
      ? 0
      : Math.min(100, Math.round((rawGrowth / (maxGrowthByPos[pos] ?? 1)) * 100));

    // ── Gem score ────────────────────────────────────────────────────────────
    const gemScore = calcGemScore(p.reliability ?? 0, efficiency, streakScore, growthScore, p.price ?? 0);

    return { ...p, efficiency, streakScore, valueScore, growthScore, gemScore };
  });
}
